...
TX_GAIN_DB  = -10                # moderat, noch nicht zu laut

ROLL_OFF    = 0.35               # RRC Roll-Off
RRC_SPAN_SYM = 8                 # Filterspanne in Symbolen (±)

# Payload
MESSAGE = b"DJ2RF"               # Test-Payload (5 byte)
...
def main():
    ...
    # 3) Convolutional FEC (1/2, K=7)
    fec_bits = conv_encode(scr_bits)
    print(f"FEC-encodierte Bits: {len(fec_bits)}")
    ...
    # 7) mehrere Wiederholungen senden
    num_repeats = 50
    ...
    print(f"Frame-Dauer: {len(tx_bb)/FS_HZ:.3f} s")
